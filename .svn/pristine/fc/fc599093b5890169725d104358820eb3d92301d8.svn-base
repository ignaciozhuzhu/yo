      ...
      if (dt.data.length == 0) {
        ...
        if (hosList.length > 2) {
          ...
          $("#hosmore").click(function() {
            location.href = "reservation_hosList.html?latitude="+latitude+"&longitude="+longitude;
          });
        }
      }
...
var searchDoc = function(searchText) {
  $.ajax({
    ...
    success: function(dt) {
      ...
      /*ReactDOM.render(
        <Search_relist data={dt.data} />,
        document.getElementById('res-doc-list')
      );*/
  ...
  $.ajax({
      ...
      success: function(dt) {
        ...
        if (dt.data.length == 0) {
          ...
          //计算距离
          let distance = 0;
          let positionx = hos.positionx;
          let positiony = hos.positiony;
          distance = getFlatternDistance(longitude, latitude, positionx, positiony);
          var htmlval = "";
          ...
          + "        <div className=\"res-doc-con-xx\">距离：<span>"+distance+"km</span></div>"
      ...
      return (
        <div className="weui_panel_bd" >
          <div className="weui_tab_bd search-doctor-list" id="res-doc-list" ref="search-doctor-list">
            <div className='text-align res-docoter-color'>未搜索到任何医生!</div>
          </div>
            </div>
      );
    } else {
