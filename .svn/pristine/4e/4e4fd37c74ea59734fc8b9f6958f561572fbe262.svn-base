...
var MobileValue = function(){//获取参数：手机号
	 ...
	 // var urlinfo= "http://localhost/newpassword.html?mobile=15958101525";                         //获取当前页面的url 
...
var NewPassWordBox = React.createClass({
  render: function() {
    return (
		<NewPassWordForm />
    );
...
var NewPassWordForm = React.createClass({
  ...
  subOnClick: function(e){
	...
	if( checkPassword(password) == true ){
	    ...
	    $.ajax({	  
			url : thisurl+"registerUser",   	 //请求的Url
			type : "post",                                           //提交方式
			...
			success : function(data){
				...
				if( data.status == "success"){
						delCookie("mobileNum");
          				setCookie("mobileNum", MobileValue(), 30);
						reminderSuccess("注册成功","login.html");
			...
			<div>
				...
				<div id="loading-toast" ref="loading-toast" className="weui_loading_toast displaynone">
		            ...
		            <div className="weui_toast">
		                ...
		                <p className="weui_toast_content text-align" id="loading-toast-text">正在注册</p>
		            </div>
		        </div>
