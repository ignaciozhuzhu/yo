...
var Search_condition = React.createClass({
	componentDidMount: function() {
		...
		ReactDOM.render(
	      <CitySelectBox  />,
	      document.getElementById('search-address-list')
	    );
		//加载智能排序列表项
		$.ajax({
	...
	disClick: function(comment) {
		$(".weui_icon_checked").attr("class", "weui_icon_checkednone");
		$(".list-list").slideUp();
		$("#mask").addClass("displaynone")
		$(".search-sel-off").removeClass("search-sel-on");
		$(".search-sel-off").parent().removeClass("show");
		if (typeof comment == "string") {
			$("#" + comment + "+.weui_icon_checkednone").attr("class", "weui_icon_checked");
			$("#" + comment + "+.weui_icon_checked").addClass("weui_icon_checkedyes");
		...
		return (
			...
			<div className="weui_navbar_bug" >
				<div className="weui_navbar_item search-sel-off" id="search-address" ref="search-address">
					<span className="" id="search-now-ads" ref="search-now-ads">{countyName}</span>
