...
var Reservation_topBox = React.createClass({
	...
	render: function() {
		return (
			<div>
        		<Reservation_docList data={this.state.data} getDoctorID={getDoctorID()} attentive={attentive} hospitalName={hospitalName} />
      		</div>
		);
	}
});
...
var Reservation_placeBox = React.createClass({
	...
	render: function() {
		return (
			<div className="weui_panel weui_panel_access res_det_box clance_border">
...
var PlaceComment = React.createClass({
	spreadClick: function() {
		var changeRes = function(text) {
			if (text == "收起") {
				$(".toggled")[0].previousSibling.children[2].innerHTML = '<span class="res_det_color"><img src="images/right_caret copy@3x.png" class="res_det_img" /></span>';
			} else if (text == "展示") {
				$(".toggled")[0].previousSibling.children[2].innerHTML = '<span class="res_det_color"><img src="images/down_caret copy@2x.png" class="res_det_img" /></span>';
			}
		}
		if ($("#" + this.props.listID).attr("class").indexOf("toggled") == -1) { //点到别的医院,就要收起小桌板
			$(".res-det-hidelist.toggled").stop().slideToggle();
			if ($(".toggled").length > 0) {
				changeRes("展示");
			}
			$(".res-det-hidelist.toggled").removeClass("toggled");
			$("#" + this.props.listID).addClass("toggled");
			$("#" + this.props.listID).stop().slideToggle();
			changeRes("收起");
		} else { //点到自己
			changeRes("展示");
	...
	render: function() {
		return (
			<div className="res_table_box">
				<div className="weui_media_title res_det_paddingBox">
					<span className="res-paddingright8px displaynone"><img src="images/Group 5@2x.png"  className="res_doctor_skill_img"  /></span>
					...
					<div className="pull-right pointer spreadClick" onClick={this.spreadClick}>
						<span className="res_det_color" ref="resdetcolor">
							<img src="images/down_caret copy@2x.png" className="res_det_img"/>
						</span>
					</div>
				</div>
				<div className="weui_media_desc res-det-padleft res-det-marbot-mx displaynone res-det-hidelist" id={this.props.listID}>
	             	...
	             	<div className="weui_media_desc text-align res-det-tableh" ref="res-resdet-table">
	                 	<div className="pull-left">
	                    	<div className="res-resdet-bortop-blue res-resdet-borleft-blue res-resdet-borbot-blue-2 text-align res-resdet-padding res-resdet-lefttable-height res_schedul_font">排<br />班</div>
	                        <div className="res-resdet-borbot-blue-2 res-resdet-borleft-blue text-align res-resdet-padding res-resdet-lefttable-height res_schedul_font">上<br />午</div>
	                        <div className="res-resdet-borbot-blue-2 res-resdet-borleft-blue text-align res-resdet-padding res-resdet-lefttable-height res_schedul_font">下<br />午</div>
	                    </div>
					...
					<div className="res_slide text-align yayi-color pointer">查看更多请向左滑动</div>
				</div>	
			</div>
		);
	}
...
var Reservation_centerHidden = React.createClass({
	componentDidMount: function() {
		...
		$.ajax({
			...
			success: function(dt) {
				...
				if (dt.status == "success") {
					console.log(JSON.stringify(dt));
					var rulesList = dt.data;
					$.each(rulesList, function(idx, item) {
						$("#res_rules_text").text("");
						$("#res_rules_text").append(
							"<p>"+
							item.booking_info.replace(/\r\n/gm,"<br />")+
							"</p>"
						);
					});
				} else {
	...
	render: function() {
		return (
			<div className="res_rules_box">
		        <div className="weui_panel weui_panel_access res_rules_panel">
		            <div className="weui_panel_bd">
		                <div className="weui_media_text res_rules_media_text">
		                    <div className="weui_media_title res_rules_media_title" id="res_rules_text"></div>
		                    <div className="res_rules_tit">预约须知</div>
		                </div>
		            </div>
		        </div>
		    </div>
...
ReactDOM.render(
	<Reservation_dialog href="reservation_resdet" getDoctorID={getDoctorID()} getHospitalID={getURLparam("hospitalID")}/>,
