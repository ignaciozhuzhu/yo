...
var DocComment = React.createClass({
	goDocdecClick: function(e) {
		...
		if (e.target.textContent == "预约") {
			location.href = "../reservation/reservation_resdet.html?doctorid=" + this.props.id + "&hospitalID=" + this.props.hospitalid;
		} else {
			location.href = "../reservation/reservation_det.html?doctorid=" + this.props.id + "&hospitalID=" + this.props.hospitalid;
		}
	},
	render: function() {
		return (
			<div className="weui_panel weui_panel_access search_noborder pointer doc-list-box" name="thisNodes" onClick={this.goDocdecClick}>
...
var HosComment = React.createClass({
	goDocClick: function() {
		if (this.props.gobackURL == "reservation_hosList") {
		...
		} else if (this.props.gobackURL == "reservation_hos_info") {
			//测试使用
			if (this.props.countycode == "" || this.props.countycode == undefined) {
				reminder("地点信息还没有选！");
				return;
			} else {
				//设置cookie
				delCookie("cityID");
				setCookie("cityID", this.props.countycode, 30);
				delCookie("gobackURL");
				setCookie("gobackURL", "reservation_hos_info.html");
				location.href = "reservation_hos_info.html?hospitalID=" + this.props.hid;
			}
		}
	},
	render: function() {
		return (
			...
			<div className="weui_panel_bd res-hos-listbg" >
				<div className="weui_media_box weui_media_appmsg res_hos_list clance_border">
					<div className="weui_media_hd hosList_imgbox">
						<img className="img-responsive center-block wx_avatar_img" src={this.props.imgs}/>
					</div>
					<div className="weui_media_bd res-listcss res_hos_det_box" name="thisNodes">
						<div className="hospital weui_media_title res-doc-name">{this.props.name}</div> 
						...
						<div className="weui_media_desc res-hos-numcolor">
							<div className="res-doc-con-xx">距离：<span>{this.props.distance}km</span></div>
							<div className="res-doc-con-yy">预约量：<span >{this.props.bookingCount}</span></div>
						</div>
					</div>
				</div>
